...
WebSocket procedures are serverless, with no additional infrastructure management. Real-time messaging uses a pluggable Pub/Sub adapter—choose your provider (e.g., Upstash Redis, native Redis, Kafka, NATS, or Cloudflare Pub/Sub). JSandy's WebSockets are designed to run on Cloudflare Workers.
