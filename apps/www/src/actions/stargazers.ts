	...
	try {
		...
		if (needExtraItems) {
			try {
				const previousPageRes = await makeRequest(
					`https://api.github.com/repos/JacobDevelops/jsandy/stargazers?per_page=20&page=${lastPage - 1}`,
				);
		...
		try {
			const lastPageRes = await makeRequest(
				`https://api.github.com/repos/JacobDevelops/jsandy/stargazers?per_page=20&page=${lastPage}`,
			);
