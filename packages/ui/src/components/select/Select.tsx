...
import { clamp } from "@jsandy/ui/number";
import {
	Popper,
	PopperAnchor,
	PopperArrow,
...
type SelectContextValue = {
	/** The current trigger element for the select component. */
	trigger: SelectTriggerElement | null;
	/** Updates the trigger element when it changes. */
	onTriggerChange(node: SelectTriggerElement | null): void;
	/** The current element that displays the selected value. */
	valueNode: SelectValueElement | null;
	/** Updates the value element when it changes. */
	onValueNodeChange(node: SelectValueElement): void;
	/** Indicates whether the value element has children. */
	valueNodeHasChildren: boolean;
	/** Updates the state of whether the value element has children. */
	...
	open: boolean;
	/** Specifies if selecting a value is required. */
	required?: boolean;
	/** Callback function triggered when the open state of the select dropdown changes. */
	onOpenChange(open: boolean): void;
	/** Specifies the text direction ('ltr' for left-to-right, 'rtl' for right-to-left). */
	dir: SelectProps["dir"];
	/** A reference to the position where the pointer was pressed on the trigger element. */
...
interface SelectProps {
	...
	/** The controlled value of the select component. */
	value?: string;

	/** The default selected value for the uncontrolled select component. */
	defaultValue?: string;

	/** Callback function triggered when the selected value changes. */
	onValueChange?(value: string): void;

	/** Controls whether the select dropdown is open. */
	open?: boolean;

	/** The default open state of the select dropdown for uncontrolled usage. */
	defaultOpen?: boolean;
	...
	/** The name of the select component, used for form submission. */
	name?: string;

	/** The autoComplete attribute for the select component. */
	autoComplete?: string;

	/** Indicates whether the select component is disabled. */
	disabled?: boolean;

	/** Specifies whether a value selection is required. */
	required?: boolean;
 ...
 *
 * The Select component provides a customizable dropdown menu for selecting options. It's designed to be flexible and accessible, supporting keyboard navigation and screen readers.
 *
 * Key features:
 * - Customizable trigger and dropdown content
 ...
 * - Use when you need users to choose one option from a list of choices
 * - Provide clear labels for the select and its options
 * - Consider using a placeholder value when no option is selected
 * - Ensure the dropdown is large enough for easy interaction on touch devices
 * - Use Select.Group and Select.Label for organizing long lists of options
 */
...
/**
 * Select.Trigger component for triggering the select dropdown
 *
 * This component renders the button that opens and closes the select dropdown. It typically contains the currently selected value and an icon indicating that it's a dropdown.
 */
...
SelectTriggerPrimitive.displayName = TRIGGER_NAME;
...
interface SelectValueProps extends Omit<PrimitiveSpanProps, "placeholder"> {
	/** The placeholder content displayed when no value is selected. */
 ...
 *
 * This component renders the currently selected value within the Select.Trigger component.
 */
...
/**
 * Select.Icon component for the dropdown icon
 *
 * This component renders an icon (typically a chevron) within the Select.Trigger to indicate that it's a dropdown.
 */
...
/**
 * Select.Content component for the dropdown content
 *
 * This component renders the dropdown menu containing the selectable options. It's conditionally rendered based on the open state of the Select.
 */
const SelectContentPrimitive = React.forwardRef<
...
SelectContentPrimitive.displayName = CONTENT_NAME;
...
type SelectContentContextValue = {
	/** The current content element for the select component. */
	content?: SelectContentElement | null;

	/** The current viewport element inside the select component. */
	viewport?: SelectViewportElement | null;

	/** Callback function triggered when the viewport element changes. */
	onViewportChange?: (node: SelectViewportElement | null) => void;

	/** Callback function for referencing individual items in the select dropdown. */
	itemRefCallback?: (
	...
	/** The currently selected item in the select dropdown. */
	selectedItem?: SelectItemElement | null;

	/** Callback function triggered when the mouse leaves a select item. */
	onItemLeave?: () => void;

	/** Callback function for referencing the text of individual select items. */
	itemTextRefCallback?: (
		_node: SelectItemTextElement | null,
		value: string,
		_disabled: boolean,
	) => void;

	/** Function to focus on the selected item in the select dropdown. */
	focusSelectedItem?: () => void;

	/** The text element of the currently selected item in the select dropdown. */
	selectedItemText?: SelectItemTextElement | null;

	/** The positioning strategy for the select content element (e.g., 'absolute', 'fixed'). */
	position?: SelectContentProps["position"];

	/** Indicates whether the select content has been positioned. */
	isPositioned?: boolean;

	/** A reference for storing search input within the select component. */
...
>((props: ScopedProps<SelectContentImplProps>, forwardedRef) => {
	...
	React.useEffect(() => {
		...
		window.addEventListener("blur-sm", close);
...
type SelectPopperPositionElement = React.ComponentRef<typeof PopperContent>;
	...
	return (
		<PopperContent
			...
			style={{
				...
				...{
					"--jsandy-select-content-transform-origin":
						"var(--jsandy-popper-transform-origin)",
					"--jsandy-select-content-available-width":
						"var(--jsandy-popper-available-width)",
					"--jsandy-select-content-available-height":
						"var(--jsandy-popper-available-height)",
					"--jsandy-select-trigger-width": "var(--jsandy-popper-anchor-width)",
					"--jsandy-select-trigger-height":
...
interface SelectViewportProps extends PrimitiveDivProps {
	/** A cryptographic nonce used for Content Security Policy (CSP) to allow inline styles or scripts. */
	nonce?: string;
}

/**
 * SelectViewport component for the scrollable viewport of the dropdown
 *
 * This component provides a scrollable container for the dropdown options, useful when there are many options to choose from.
 */
		...
		<>
			...
			<style
				...
				dangerouslySetInnerHTML={{
					__html:
						"[data-jsandy-select-viewport]{scrollbar-width:none;-ms-overflow-style:none;-webkit-overflow-scrolling:touch;}[data-jsandy-select-viewport]::-webkit-scrollbar{display:none}",
				}}
				nonce={nonce}
			/>
			<Collection.Slot scope={__scopeSelect}>
				<Primitive.div
					data-jsandy-select-viewport=""
					role="presentation"
					{...viewportProps}
					ref={composedRefs}
					style={{
 ...
 *
 * This component is used to group related select options together, providing a logical separation within the dropdown.
 */
...
/**
 * Select.Label component for labeling groups of select items
 *
 * This component is used to provide a label or header for a group of related select options, improving the organization and readability of the dropdown.
 */
const SelectLabelPrimitive = React.forwardRef<
	SelectLabelElement,
	SelectLabelProps
...
SelectLabelPrimitive.displayName = LABEL_NAME;
...
type SelectItemContextValue = {
	...
	/** Indicates whether the select item is disabled. */
	disabled: boolean;

	/** The unique identifier for the select item's text element. */
	textId: string;

	/** Indicates whether the select item is currently selected. */
	isSelected: boolean;

	/** Callback function triggered when the select item's text changes. */
...
interface SelectItemProps extends PrimitiveDivProps {
	...
	/** The text value of the select item, used for accessibility and searching. */
	textValue?: string;
}

/**
 * SelectItem component for individual items in the select dropdown
 *
 * This component renders a single selectable option within the dropdown. It handles the selection state and provides visual feedback for hover and selection.
 */
...
SelectItemPrimitive.displayName = ITEM_NAME;
...
/**
 * SelectItemText component for the text content of a select item
 *
 * This component renders the text label for a select option.
 */
...
/**
 * SelectItemIndicator component for indicating the selected item
 *
 * This component renders a checkmark or other indicator for the currently selected option.
 */
...
const SelectScrollUpButtonPrimitive = React.forwardRef<
...
SelectScrollUpButtonPrimitive.displayName = SCROLL_UP_BUTTON_NAME;
...
const SelectScrollDownButtonPrimitive = React.forwardRef<
...
SelectScrollDownButtonPrimitive.displayName = SCROLL_DOWN_BUTTON_NAME;
...
/**
 * Select.Separator component for separating groups of items in the select dropdown
 *
 * This component renders a horizontal line to visually separate different sections or groups within the dropdown.
 */
const SelectSeparatorPrimitive = React.forwardRef<
...
SelectSeparatorPrimitive.displayName = SEPARATOR_NAME;
...
type SelectArrowElement = React.ComponentRef<typeof PopperArrow>;
...
const SelectTrigger = React.forwardRef<
	React.ComponentRef<typeof SelectTriggerPrimitive>,
...
const SelectScrollUpButton = React.forwardRef<
	React.ComponentRef<typeof SelectScrollUpButtonPrimitive>,
...
const SelectScrollDownButton = React.forwardRef<
	React.ComponentRef<typeof SelectScrollDownButtonPrimitive>,
...
const SelectContent = React.forwardRef<
	React.ComponentRef<typeof SelectContentPrimitive>,
		...
		>
			...
			<SelectViewport
				className={cn(
					"p-1",
					position === "popper" &&
						"h-[var(--jsandy-select-trigger-height)] w-full min-w-[var(--jsandy-select-trigger-width)]",
				)}
			>
				{children}
			</SelectViewport>
...
const SelectLabel = React.forwardRef<
	React.ComponentRef<typeof SelectLabelPrimitive>,
...
const SelectItem = React.forwardRef<
	React.ComponentRef<typeof SelectItemPrimitive>,
...
const SelectSeparator = React.forwardRef<
	React.ComponentRef<typeof SelectSeparatorPrimitive>,
