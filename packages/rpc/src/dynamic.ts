...
export const dynamic = <T extends Router>(
	...
	return async () => {
		...
		if (!(router instanceof Router)) {
			throw new Error(
				`Error dynamically loading router: Invalid router module - Expected exported value to be a Router instance, but received ${router === null ? "null" : typeof router}. Are you exporting multiple functions from this file?`,
			);
		}
